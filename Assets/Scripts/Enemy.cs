...
namespace Completed
{
    ...
    public class Enemy : MonoBehaviour
    {
        ...
        public void MoveEnemy()
        {
            ...
            {
                bool TF = false;
                TF = Random.Range(0, 1) == 0 ? false : true;
            ...
            if (moveVertical)
            {
                ...
                if (moveUp)
                {
                    move = new Vector2(0, 1);// coordenades (x,y) para arriba
                    animator.SetFloat("movX", move.x);
                    animator.SetFloat("movY", move.y);
                }
                else
                {
                    move = new Vector2(0, -1); //xy para abajo
                    animator.SetFloat("movX", move.x);
                    animator.SetFloat("movY", move.y);
            ...
            if (moveHorizontal)
            {
                ...
                if (moveRight)
                {
                    move = new Vector2(1, 0);// coordenades (x,y) para derecha
                    animator.SetFloat("movX", move.x);
                    animator.SetFloat("movY", move.y);
                ...
                {
                    move = new Vector2(-1, 0); //xy para izquierda
                    animator.SetFloat("movX", move.x);
                    animator.SetFloat("movY", move.y);
                }
            }
